...
uniform vec2 noise_scale = vec2(5.0, 10.0);
uniform int noise_seed = 0;
uniform float noise_motion_speed = 1;
...
void fragment()
{
	...
	float noise_influence = noise_amount * cnoise(vec3((UV + float(noise_seed))*noise_scale, TIME*noise_motion_speed));
	ALBEDO = mixRGB(texture(SCREEN_TEXTURE, SCREEN_UV).xyz, fog_color.xyz,
	 linear_depth/(max_distance-min_distance) + noise_influence);
}